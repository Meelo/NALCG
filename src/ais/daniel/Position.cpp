...
void Position::move(std::string mv) {
  int i1 = 0;
  int j1 = 0;
  int i2 = 0;
  int j2 = 0;
  if (mv.at(0) == 'O') {
    ...
    // Convert <letter><number> coordinates to array coordinates
    j1 = mv.at(0) - 'A';
    i1 = 8 - (mv.at(1) - '0');
    j2 = mv.at(3) - 'A';
    i2 = 8 - (mv.at(4) - '0');
    ...
    case 'x':
    case 'X':
      board[i2][j2] -= 13; // X -> K, x -> k
      break;
    case 't':
    case 'T':
      board[i2][j2] -= 2; // T -> R, t -> r
      break;
    case 'p':
    case 'P':
      if (std::abs(i1 - i2) > 1) {
...
std::vector<std::string>* Position::getLegalMoves() {
  return &legalMoves;
}
