...
void MovementGenerator::addLegalRookMoves(int i, int j) {
  ...
  while (x >= 0) {
    if ((board[i][x] == ' ' || isPieceHostile(i,x)) && isKingSafe(i,j,i,x)) {
      legalMoves->push_back(convertToStringCoordinates(i,j,i,x));
    }
    if (board[i][x] != ' ') {
      break;
    }
  ...
  while (y >= 0) {
    if ((board[y][j] == ' ' || isPieceHostile(y,j)) && isKingSafe(i,j,y,j)) {
      legalMoves->push_back(convertToStringCoordinates(i,j,y,j));
    }
    if (board[y][j] != ' ') {
      break;
    }
...
void MovementGenerator::addLegalBishopMoves(int i, int j) {
  ...
  while (x >= 0 && y >= 0) {
    if ((board[y][x] == ' ' || isPieceHostile(y,x)) && isKingSafe(i,j,y,x)) {
      legalMoves->push_back(convertToStringCoordinates(i,j,y,x));
    }
    if (board[y][x] != ' ') {
      break;
    }
...
void MovementGenerator::addLegalPawnMoves(int i, int j) {
  ...
  if (board[i][j] == 'p') {
	...
	legalMoves->push_back(convertToStringCoordinates(i,j,i-2,j));
      }
    }
  }
  else {
...
void MovementGenerator::getAllLegalMoves(std::vector<std::string> *lm) {
  legalMoves = lm;
  for (int i = 0; i < 8; ++i) {
    for (int j = 0; j < 8; ++j) {
